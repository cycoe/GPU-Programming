    ...
    for(unsigned int i=0; i<np; i++) 
        ...
        for(unsigned int j=0; j<np; j++) 
            ...
            if (r < rcut) 
                ...
                float epot = 0.5*4.0*epsilonij*(pow(sigmaij/r,12.0) - pow(sigmaij/r,6.0));             // potential between particle i and j
...
//	float temperature = 1.0;                // target temperature
	...
	float3 box =float3(15.0, 15.0, 15.0);     // box size in x, y, and z directions
