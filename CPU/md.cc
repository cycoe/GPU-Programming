  ...
  for (unsigned int i = 0; i < np; i++) {
    ...
    for (unsigned int j = 0; j < np; j++) {
      ...
      if (r < rcut) {
        ...
        float ffac =
            -4.0 * epsilonij *
            (-12.0 * pow(sigmaij / r, 12.0) / r +
             6.0 * pow(sigmaij / r, 6.0) / r); // force between particle i and j
        float epot =
            0.5 * 4.0 * epsilonij *
            (pow(sigmaij / r, 12.0) -
             pow(sigmaij / r, 6.0)); // potential between particle i and j
...
int main(int argc, char **argv) {
  ...
  unsigned int nsteps = 50;  // the number of time steps
  ...
  float3 epsilon =
      float3(1.0, 0.5, 1.0); // epsilon.x for type 1.0 and 1.0; epsilon.y for
                             // type 1.0 and 2.0; epsilon.z for type 2.0 and 2.0
  float3 sigma =
      float3(1.0, 1.0, 1.0); // sigma.x for type 1.0 and 1.0; sigma.y for
                             // type 1.0 and 2.0; sigma.z for type 2.0 and 2.0
  float min_dis =
      sigma.x * 0.9; // the minimum distance between particles for system generation
  ...
  for (unsigned int step = 0; step <= nsteps; step++) // running simulation loop
  {
    ...
    if ((step % nprint) == 0) {
      ...
      printf("time step %02d, Temperature: %.2f, Energy: %.2f\n", step, info[0], info[2]);
    }
  }
